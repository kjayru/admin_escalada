...
>
    ...
    <div class="fi-media-item-thumbnail-container">
        ...
        @else
            ...
            @else
                {{-- Área vacía con badge de extensión para archivos no-imagen --}}
                <div class="flex flex-col items-center justify-center h-full bg-gray-50 dark:bg-gray-900/50">
                    <span
                        class="px-2 py-1 rounded-md bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold uppercase">
                        {{ $item->extension ?? 'file' }}
                    </span>
                </div>
            @endif
        ...
        <div class="absolute inset-0 z-10 cursor-pointer"
             style="position: absolute; top: 0; right: 0; bottom: 0; left: 0; z-index: 10; cursor: pointer;"
    ...
    <div class="fi-media-item-info">
        <div class="min-w-0">
            <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate flex items-center gap-1"
                title="{{ $item->name }}">
                @if($isFolder)
                    <x-heroicon-s-folder class="w-2.5 h-2.5 flex-shrink-0 text-amber-500" />
                @else
                    <x-heroicon-s-document class="w-2.5 h-2.5 flex-shrink-0 text-gray-400" />
                @endif
